...
//   $isModal = 1;
// }
echo $this->Form->create('Gatepass',array('class'=>'form-horizontal','enctype'=>'multipart/form-data','url' => '/PrisonerTransfers/gatepassList'));?>
<?php if($isModal == 1)
{?>
  <!-- Verify Modal START -->
    ...
    <thead>
        ...
        <th>
          ...
          ?>
            ...
            <th>
           <?php
            echo $this->Paginator->sort('PrisonerTransfer.transfer_date','Date Of Transfer',array('update'=>'#listingDiv','evalScripts' => true,'url'=>array('controller' => 'PrisonerTransfers','action' => 'gatepassListAjax','prisoner_id'   => $prisoner_id,
            // 'date_from'     => $date_from,
            // 'date_to'       => $date_to,
        ...
        ?>
            <th style="text-align: left;width:15%">Gatepass Details</th>
        </tr>
    </thead>
    <tbody>
<?php
...
foreach($datas as $data){//debug($data);
  $display_status = Configure::read($data['PrisonerTransfer']['status']);
  $prisonerDetails = $funcall->getPrisonerDetails($data['PrisonerTransfer']['prisoner_id']);
?>
        <tr>
        <?php
        if(!isset($is_excel)){
          ?>
            ...
            if(count($data['Gatepass'])==0){
              echo $this->Form->input('Gatepass.'.$rowCnt.'.fid', array(
                      'type'=>'checkbox', 'value'=>$data['PrisonerTransfer']['id'],'hiddenField' => false, 'label'=>false,
                      'format' => array('before', 'input', 'between', 'label', 'after', 'error' ) 
                  ));
            }
            ?>
...
function MyYesFunction() {
  if(isModal == 1)
    ...
    {
      $('#GatepassGatepassListAjaxForm').submit();
    }
}
