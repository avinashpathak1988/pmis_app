                        ...
                        <div class="span6">
                            ...
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    ...
                                    echo $this->Form->input('escorting_officer',array('div'=>false,'label'=>false,'class'=>'form-control span11','type'=>'select','options'=>array(1=>"Canceled",0=>"Not Canceled"), 'empty'=>'-- All Status --','required','id'=>'statusaa',"default"=>$status));?>
                                </div>
                            </div>
                        </div>
                        <div class="span6">
...
<?php
$ajaxCancelUrl    = $this->Html->url(array('controller'=>'PrisonerTransfers','action'=>'transferCancel'));
$ajaxUrl    = $this->Html->url(array('controller'=>'PrisonerTransfers','action'=>'transferCancelListAjax'));
...
echo $this->Html->scriptBlock("
    ...
    function checkTransferCancel(id){
        var paramId = $('#verify_id'+id).val();
        var cancel_remarks = $('#cancel_remarks'+id).val();
        var prisoner_id = $('#prisoner_id'+id).val();
        AsyncConfirmYesNo(
            'Are you sure want to cancel this transfer?',
            'Yes',
            'No',
            function(){
                var url = '".$ajaxCancelUrl."';
                $.post(url, {'paramId':paramId,'cancel_remarks':cancel_remarks,'prisoner_id':prisoner_id}, function(res) { 
                    if(res.trim() == 'SUCC'){
                        $('#verify'+id).modal('hide');
                        $('#verify_remark'+id).val('');
                        showData();
