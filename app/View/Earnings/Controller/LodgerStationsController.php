...
class LodgerStationsController    extends AppController {
                    ...
                    if($this->Session->read('Auth.User.usertype_id')==Configure::read('RECEPTIONIST_USERTYPE'))
                    {
                        ...
                        if(isset($notifyUser['User']['id']))
                        {
                            $this->addNotification(array( 
                                ...
                                "url_link"   => "LodgerStations/index/".$lodger_type,
                            )); 
                        }
                    }
    ...
    public function getLodgerData($id){
        ...
        return $this->LodgerStation->findByParentId($id);
    }

    // updateing lodger out out
    public function updateOut(){
        $this->request->data['date_of_lodging'] = date("Y-m-d H:i:s");
        $this->request->data['status'] = 'Draft';
        // debug($this->request->data);exit;
        if($this->LodgerStation->saveAll($this->request->data)){
            echo "SUCC";exit;
        }else{
            echo "FAIL";exit;
        }
