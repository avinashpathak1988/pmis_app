        ...
        <div class="span12">
            <div class="widget-box">
                <div class="widget-title"> <span class="icon"> <i class="icon-align-justify"></i> </span>
                    <h5>Lodger Out List</h5>
                    <div style="float:right;padding-top: 3px;">
                        <?php 
                        if($this->Session->read('Auth.User.usertype_id') == Configure::read("RECEPTIONIST_USERTYPE")){
                            echo $this->Html->link('Add New',array('controller'=>'Lodgers','action'=>'lodgerOutAdd'),array('escape'=>false,'class'=>'btn btn-success btn-mini')); 
                        }
                        ?>
                        &nbsp;&nbsp;
                    </div>
...
<?php
$ajaxUrl            = $this->Html->url(array('controller'=>'Lodgers','action'=>'lodgerOutAjax'));
$ajaxUpdateOutUrl   = $this->Html->url(array('controller'=>'Lodgers','action'=>'lodgerOutSave'));
echo $this->Html->scriptBlock("
    ...
    { 
        if(lodger_station_id != '')
        {
            ...
            $.post(strURL,{'reason':$('#reason'+lodger_station_id).val(),'stay_duration':$('#stay_duration'+lodger_station_id).val(),'release_type':$('#release_type'+lodger_station_id).val(),'lodger_id':lodger_station_id},function(data){  
                showData();
                $('.modal-backdrop').hide();
            });
        }
