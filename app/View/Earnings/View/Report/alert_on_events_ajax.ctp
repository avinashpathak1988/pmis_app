...
?>
<?php
    $exUrl = "alertOnEventsAjax/name:$name";
...
<?php $count++; ?>
	<tr>
			...
			<td><?php echo $PrisonerVal['PrisonerCaseFile']['case_file_no'];?></td>
			<td><?php echo $funcall->getPrisonerCourt($PrisonerVal['Prisoner']['id']);?></td>	
			<td><?php echo $funcall->getName($PrisonerVal['IncidentManagement']['incident_type'],'IncidentType','incident_name');?></td>
			<td><?php echo date(Configure::read('UGANDA-DATE-FORMAT'),strtotime($PrisonerVal['IncidentManagement']['date']));  ?></td>
			<td></td>
			<td><?php echo ucwords(h($PrisonerVal['IncidentManagement']['remarks'])); ?>&nbsp;</td>	
