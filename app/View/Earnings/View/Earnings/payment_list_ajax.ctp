<?php //echo '<pre>'; print_r($PrisonerPaymentList); exit;
...
<table class="table table-bordered data-table table-responsive">
    <thead>
        <tr>
            ...
            <th>
                <?php 
                echo $this->Paginator->sort('Prisoner.prisoner_no','Prisoner No',array('update'=>'#listingDiv','evalScripts' => true,'url'=>array('controller' => 'Earnings','action' => 'paymentListAjax')));
                ?>
            </th>
            <th>
                <?php 
                echo $this->Paginator->sort('Prisoner.name','Prisoner Name',array('update'=>'#listingDiv','evalScripts' => true,'url'=>array('controller' => 'Earnings','action' => 'paymentListAjax')));
                ?>
            </th>
            <th>
                <?php 
                echo $this->Paginator->sort('PrisonerPayment.start_date','Start Date',array('update'=>'#listingDiv','evalScripts' => true,'url'=>array('controller' => 'Earnings','action' => 'paymentListAjax')));
                ?>
            </th>
            <th>
                <?php 
                echo $this->Paginator->sort('PrisonerPayment.end_date','End Date',array('update'=>'#listingDiv','evalScripts' => true,'url'=>array('controller' => 'Earnings','action' => 'paymentListAjax')));
                ?>
            </th>
            <th>Pay Amount</th>
...
<?php
    ...
    foreach($datas as $data){
        ...
        <tr class="<?php if($rowCnt == count($datas)) {echo 'lastrow';}?>">
            ...
            <td><?php echo date('d-m-Y', strtotime($data['PrisonerPayment']['start_date'])); ?></td>
            <td><?php echo date('d-m-Y', strtotime($data['PrisonerPayment']['end_date'])); ?></td>

            <td><?php echo $data['PrisonerPayment']['pay_amount']; ?></td>
            <td>
...
function MyYesFunction() {
  if(isModal == 1)
    ...
    {
      $('#ApprovalProcessFormPaymentListAjaxForm').submit();
    }
}
