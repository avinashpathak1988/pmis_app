...
$methodName = 'approveWorkingPartyRejects';
$modelName = 'WorkingPartyReject';    
...
<table class="table table-bordered data-table table-responsive">
    <thead>
		...
		<?php } ?>
            ...
            <th>
                <?php 
                echo $this->Paginator->sort('WorkingPartyReject.rejection_date','Rejection Date',array('update'=>'#listingDiv','evalScripts' => true,'url'=>array('controller' => 'Earnings','action' => 'approveWorkingPartyRejects')));
                ?>
            </th>
            <th>
                <?php 
                echo $this->Paginator->sort('WorkingPartyReject.rejection_remark','Rejection Remark',array('update'=>'#listingDiv','evalScripts' => true,'url'=>array('controller' => 'Earnings','action' => 'approveWorkingPartyRejects')));
                ?>
            </th>
            <th>Prisoner Id</th>
...
<?php
    ...
    foreach($datas as $data){
    	//debug($data);
        $id = $data['WorkingPartyReject']['id'];
        //$prisoner_id = $data['Prisoner']['id'];
        $display_status = Configure::read($data[$modelName]['status']);
        $transfered_prisoners = $funcall->getPrisonerNos($data['WorkingPartyReject']['prisoner_id']);
        ?>
        <tr class="<?php if($rowCnt == count($datas)) {echo 'lastrow';}?>">
            ...
            <td><?php echo date('d-m-Y', strtotime($data['WorkingPartyReject']['rejection_date'])); ?></td>
            <td><?php echo $data['WorkingPartyReject']['rejection_remark']; ?></td>
...
<?php
    $this->Paginator->options(array(
        ...
        'url'                       => array(
            'controller'            => 'Earnings',
            'action'                => 'approveWorkingPartyRejects'
...
function MyYesFunction() {
  if(isModal == 1)
    ...
    {
      $('#ApprovalProcessFormWorkingPartyRejectListForm').submit();
    }
}
