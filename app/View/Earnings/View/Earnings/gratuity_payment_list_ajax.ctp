<?php //echo '<pre>'; print_r($PrisonerSavingList); exit;
...
$methodName = 'approveGratuityPayments';
$modelName = 'PrisonerSaving';    
...
<table class="table table-bordered data-table table-responsive">
    <thead>
		...
		<?php
            ...
            <th>
                <?php 
                echo $this->Paginator->sort('Prisoner.prisoner_no','Prisoner No',array('update'=>'#listingDiv','evalScripts' => true,'url'=>array('controller' => 'Earnings','action' => 'gratuityPaymentListAjax')));
                ?>
            </th>
            <th>
                <?php 
                echo $this->Paginator->sort('Prisoner.name','Prisoner Name',array('update'=>'#listingDiv','evalScripts' => true,'url'=>array('controller' => 'Earnings','action' => 'gratuityPaymentListAjax')));
                ?>
            </th>
            <th>
                <?php 
                echo $this->Paginator->sort('PrisonerSaving.created','Pay Date',array('update'=>'#listingDiv','evalScripts' => true,'url'=>array('controller' => 'Earnings','action' => 'gratuityPaymentListAjax')));
                ?>
            </th>
            <th>Pay Amount</th>
...
<?php
    ...
    foreach($datas as $data){
        ...
        <tr class="<?php if($rowCnt == count($datas)) {echo 'lastrow';}?>">
            ...
            <td><?php echo date('d-m-Y', strtotime($data['PrisonerSaving']['created'])); ?></td>

            <td><?php echo $data['PrisonerSaving']['amount']; ?></td>
            <td>
...
<?php
    $this->Paginator->options(array(
        ...
        'url'                       => array(
            'controller'            => 'Earnings',
            'action'                => 'gratuityPaymentListAjax'
...
function MyYesFunction() {
  if(isModal == 1)
    ...
    {
      $('#ApprovalProcessFormGratuityPaymentListAjaxForm').submit();
    }
}
