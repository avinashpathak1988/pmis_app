            ...
            <div class="widget-box">
                <div class="widget-title"> <span class="icon"> <i class="icon-align-justify"></i> </span>
                    ...
                    <div style="float:right;padding-top: 7px;">
                        <?php echo $this->Html->link(__('Add new'), array('action' => 'addSpiritualAndMoralRehabiliation'), array('escape'=>false,'class'=>'btn btn-success btn-mini')); ?>
                        &nbsp;&nbsp;
                    </div>
                </div>
                 ...
                 <div class="widget-content nopadding">
                    ...
                    <?php echo $this->Form->create('Search',array('class'=>'form-horizontal'));?>
                        <div id="searchPrisonerTwo" class="row collapse" style="height:auto;">
                          ...
                          <div class="span12 add-top" align="center" valign="center">
                                ...
                                <?php echo $this->Form->input('Reset', array('type'=>'reset', 'div'=>false,'label'=>false, 'class'=>'btn btn-danger', 'onclick'=>"resetData('SearchSpiritualAndMoralRehabiliationForm')"))?>
                            </div> 
                    </div> 
                                <?php echo $this->Form->end();?>
                    </div>
                    <div id="SpiritualAndMoralList"></div>
                    <div id="addHeadRemarkModal" class="modal fade" role="dialog">
                        <div class="modal-dialog">
                            ...
                            <?php echo $this->Form->input('id',array('div'=>false,'label'=>false,'class'=>'form-control ','id'=>'spritual_id','type'=>'hidden'));?>
...
$ajaxUrlSpiritualAndMoralList = $this->Html->url(array('controller'=>'SocialRehabiliationProgramme','action'=>'spiritualAndMoralRehabilitaionAjax'));
...
$( document ).ready(function() {
            ...
            var url ='<?php echo $ajaxUrlSpiritualAndMoralList?>';
            $.post(url,{},function(res) {
                if (res) {
                    $('#SpiritualAndMoralList').html(res);
                    
                 }else{
                    
                    $('#SpiritualAndMoralList').html('No records found !');
     ...
     function showListSearch(){
        ...
        var url ='<?php echo $ajaxUrlSpiritualAndMoralList?>';
        $.post(url, $('#SearchSpiritualAndMoralRehabiliationForm').serialize(), function(res) {
            if (res) {
                        $('#SpiritualAndMoralList').html(res);
                 }else{
                        $('#SpiritualAndMoralList').html('No records found !');
...
function submitHeadRemark(){
    var url ='<?php echo $ajaxUrlsubmitHeadRemark?>';
        $.post(url,$('#SpiritualMoralRehabilationSpiritualAndMoralRehabiliationForm').serialize(), function(res) {
                if (res == 'success') {
 ...
 function discontinueItem(id){
    ...
    var url ='<?php echo $ajaxUrlDiscontinue?>';
        $.post(url,{'id':id,'model':'SpiritualMoralRehabilation'}, function(res) {
                if (res == 'success') {
