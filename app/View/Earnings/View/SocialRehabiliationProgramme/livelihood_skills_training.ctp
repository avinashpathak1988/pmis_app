            ...
            <div class="widget-box">
                <div class="widget-title"> <span class="icon"> <i class="icon-align-justify"></i> </span>
                    ...
                    <div style="float:right;padding-top: 7px;">
                        <?php echo $this->Html->link(__('Add New'), array('action' => 'addLivelihoodSkillsTraining'), array('escape'=>false,'class'=>'btn btn-success btn-mini')); ?>
                        &nbsp;&nbsp;
                    </div>
                </div>
                    ...
                    <?php echo $this->Form->create('Search',array('class'=>'form-horizontal'));?>
                        <div id="searchPrisonerTwo" class="row collapse" style="height:auto;">
                          ...
                          <div class="span12 add-top" align="center" valign="center">
                                ...
                                <?php echo $this->Form->input('Reset', array('type'=>'reset', 'div'=>false,'label'=>false, 'class'=>'btn btn-danger', 'onclick'=>"resetData('SearchLivelihoodSkillsTrainingForm')"))?>
                            </div> 
                    </div> 
                                <?php echo $this->Form->end();?>
                    </div>
                    <div id="LivelihoodSkillsTrainingList"></div>
                     <div id="addHeadRemarkModal" class="modal fade" role="dialog">
                        <div class="modal-dialog">
                            ...
                            <?php echo $this->Form->input('id',array('div'=>false,'label'=>false,'class'=>'form-control ','id'=>'livelihood_id','type'=>'hidden'));?>
...
$ajaxUrlLivelihoodSkillsTrainingList = $this->Html->url(array('controller'=>'SocialRehabiliationProgramme','action'=>'livelihoodSkillsTrainingAjax'));
...
<script type="text/javascript">
    ...
    $( document ).ready(function() {
        ...
        var url ='<?php echo $ajaxUrlLivelihoodSkillsTrainingList?>';
            $.post(url,{},function(res) {
                if (res) {
                    $('#LivelihoodSkillsTrainingList').html(res);
                    
                 }else{
                    
                    $('#LivelihoodSkillsTrainingList').html('No records found !');
 ...
 function showListSearch(){
        ...
        var url ='<?php echo $ajaxUrlLivelihoodSkillsTrainingList?>';
        $.post(url, $('#SearchLivelihoodSkillsTrainingForm').serialize(), function(res) {
            if (res) {
                        $('#LivelihoodSkillsTrainingList').html(res);
                 }else{
                        $('#LivelihoodSkillsTrainingList').html('No records found !');
...
function finalSave(id){
        ...
        $.post(url,{'id':id,'model':'LivelihoodSkillsTraining'}, function(res) {
                if (res == 'success') {
...
function submitHeadRemark(){
    var url ='<?php echo $ajaxUrlsubmitHeadRemark?>';
        $.post(url,$('#LivelihoodSkillsTrainingLivelihoodSkillsTrainingForm').serialize(), function(res) {
                if (res == 'success') {
                    //dynamicAlertBox('Message', 'continued successfully !');
                    location.reload();
                }else{
