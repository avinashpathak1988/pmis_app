            ...
            <div class="widget-box">
                <div class="widget-title"> <span class="icon"> <i class="icon-align-justify"></i> </span>
                    ...
                    <div style="float:right;padding-top: 7px;">
                        <?php echo $this->Html->link(__('Specific case treatment list'), array('action' => 'specificCaseTreatment'), array('escape'=>false,'class'=>'btn btn-success btn-mini')); ?>
                        &nbsp;&nbsp;
                    </div>
                </div>
                        ...
                        <div class="aftercareform">
                            ...
                            <span class="span12">
                                <div class="form-actions" align="center">
                                    <button type="button" id="SpecificCaseTreatmentSaveBtn" class="btn btn-success formSaveBtn" formnovalidate="true", onclick="submitSpecificCaseTreatment()">Save</button>
                                    <?php echo $this->Form->button('Reset', array('type'=>'button', 'div'=>false,'label'=>false, 'class'=>'btn btn-danger formResetBtn','onclick'=>"resetData('SpecificCaseTreatmentAddSpecificCaseTreatmentForm')", 'formnovalidate'=>true))?>
                                </div>
                            </span>
                                </div>
...
$ajaxUrlSubmitSpecificCaseTreatment = $this->Html->url(array('controller'=>'SocialRehabiliationProgramme','action'=>'saveSpecificCaseTreatment'));
...
<script type="text/javascript">
    ...
    function submitSpecificCaseTreatment(){
        if($("#SpecificCaseTreatmentAddSpecificCaseTreatmentForm").valid()){
            var prisonerID = $('#prisoner_no').val();
            var url ='<?php echo $ajaxUrlSubmitSpecificCaseTreatment?>';

        
            $.post(url, $('#SpecificCaseTreatmentAddSpecificCaseTreatmentForm').serialize(), function(res) {
                if (res.trim()=='Success') {
                    dynamicAlertBox('Message', 'Specific case Treatment saved successfully !');
                    //showListSearch();
                    //resetForm('AftercareIndexForm');
                    window.location = '<?php echo $this->webroot;?>SocialRehabiliationProgramme/specificCaseTreatment';
                }else{
                    dynamicAlertBox('Message', 'Specific Case Treatment  not saved !');
                }
            });
        }
...
$(function(){
        $("#SpecificCaseTreatmentAddSpecificCaseTreatmentForm").validate({
     
            ignore: "",
                rules: {  
                    
                    'data[SpecificCaseTreatment][program_head_id]': {
                        required: true,
                    },
                    'data[SpecificCaseTreatment][prisoner_no]': {
                        required: true,
                    },
                    'data[SpecificCaseTreatment][date_of_enrolment]': {
                        required: true,
                    },
                    'data[SpecificCaseTreatment][start_date]': {
                        required: true,
                    },
                    'data[SpecificCaseTreatment][prisoner_name]': {
                        required: true,
                    },
                    'data[SpecificCaseTreatment][end_date]': {
                        required: true,
                    },
                    'data[SpecificCaseTreatment][prisoners_input]':{
                        required: true,
                    },
                    'data[SpecificCaseTreatment][themes][]':{
                        required: true,
                    },
