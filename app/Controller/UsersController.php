 ...
 */            
    ...
    public function ugandaimport(){
        ...
        // echo WWW_ROOT.DS;exit;
        $data->read(WWW_ROOT.'/files/aaaa.xls');
        //debug($data->sheets[0]["cells"]);
        for ($x = 2; $x<=count($data->sheets[0]["cells"]); $x++) {
            $a='';
            //$geo_id = (isset($data->sheets[0]["cells"][$x][1]) && $data->sheets[0]["cells"][$x][1] !="") ? trim($data->sheets[0]["cells"][$x][1]) :"";

            /////////////////////////////////////////////district/////////////////////////////////////////

            $district = (isset($data->sheets[0]["cells"][$x][2]) && $data->sheets[0]["cells"][$x][2] !="") ? trim($data->sheets[0]["cells"][$x][2]) :"";
            $disArr = array();
            $countyArr=array();
            $subCountyArr=array();
            $parishArr=array();
            $villageArr=array();
            if($district !=''){
                $a= $this->getDistId($district,'District');
                //debug($a['District']['id']);
                //$disId=$a['District']['id'];
                if(!$a){
                    $disArr = array(
                    ...
                    $this->District->saveAll($disArr);
                    $districtId = $this->District->id;
                    $county = (isset($data->sheets[0]["cells"][$x][3]) && $data->sheets[0]["cells"][$x][3] !="") ? trim($data->sheets[0]["cells"][$x][3]) :"";
                    if($county !=''){
                        $a1= $this->getCountyId($districtId,'County',$county);
                        //debug($a1);exit;
                        if(!$a1){
                            $countyArr = array(
                                        'name'  => $county,
                            ...
                            //debug($countyArr);
                            $this->County->saveAll($countyArr);
                            $countyId = $this->County->id;
                            //////////////////////////////////////Sub County///////////////////////////////////////////

                            $subCounty = (isset($data->sheets[0]["cells"][$x][4]) && $data->sheets[0]["cells"][$x][4] !="") ? trim($data->sheets[0]["cells"][$x][4]) :"";
                            if($subCounty !=''){
                                $a3= $this->getSubCountyId($districtId,'SubCounty',$countyId,$subCounty);
                                //debug($a);
                                if(!$a3){
                                    $subCountyArr = array(
                                                'name'  => $subCounty,
                                    ...
                                    debug($subCountyArr);
                                    $this->SubCounty->saveAll($subCountyArr);
                                    $subcountyId = $this->SubCounty->id;
                                    /////////////////////////////////////////Parish////////////////////////////////////////

                                        $parish = (isset($data->sheets[0]["cells"][$x][5]) && $data->sheets[0]["cells"][$x][5] !="") ? trim($data->sheets[0]["cells"][$x][5]) :"";
                                        if($parish !=''){
                                            $a5= $this->getSubCountyId($districtId,'Parish',$county_id,$subcountyId,$parish);
                                            //debug($a5);
                                            if(!$a5){
                                                $parishArr = array(
                                                        'name'  => $parish,
                                                ...
                                                debug($parishArr);
                                                $this->Parish->saveAll($parishArr);
                                                $parishId = $this->Parish->id;
                                                $village = (isset($data->sheets[0]["cells"][$x][6]) && $data->sheets[0]["cells"][$x][6] !="") ? trim($data->sheets[0]["cells"][$x][6]) :"";
                                                if($village !=''){
                                                    $a6= $this->getSubCountyId($districtId,'Village',$county_id,$subcountyId,$parishId,$village);
                                                    //debug($a6);
                                                    if(!$a6){
                                                        $villageArr = array(
                                                                'name'  => $village,
                                                        ...
                                                        debug($villageArr);
                                                        $this->Village->saveAll($villageArr);
                                                        $villageId = $this->Village->id;
                                                    }else{
                                                        echo 'Village already exist in row no '.$x.' of Village column';
                                                    }
                                                ...
                                                $village = (isset($data->sheets[0]["cells"][$x][6]) && $data->sheets[0]["cells"][$x][6] !="") ? trim($data->sheets[0]["cells"][$x][6]) :"";
                                                if($village !=''){
                                                    $a6= $this->getSubCountyId($districtId,'Village',$county_id,$subcountyId,$a5,$village);
                                                    //debug($a6);
                                                    if(!$a6){
                                                        $villageArr = array(
                                                                'name'  => $village,
                                    ...
                                    $parish = (isset($data->sheets[0]["cells"][$x][5]) && $data->sheets[0]["cells"][$x][5] !="") ? trim($data->sheets[0]["cells"][$x][5]) :"";
                                    if($parish !=''){
                                        $a5= $this->getSubCountyId($districtId,'Parish',$county_id,$a3,$parish);
                                        //debug($a5);
                                        if(!$a5){
                                            $parishArr = array(
                                                    'name'  => $parish,
                            ...
                            $subCounty = (isset($data->sheets[0]["cells"][$x][4]) && $data->sheets[0]["cells"][$x][4] !="") ? trim($data->sheets[0]["cells"][$x][4]) :"";
                            if($subCounty !=''){
                                $a3= $this->getSubCountyId($districtId,'SubCounty',$a1,$subCounty);
                                //debug($a);
                                if(!$a3){
                                    $subCountyArr = array(
                                                'name'  => $subCounty,
                                    ...
                                    /////////////////////////////////////////Parish////////////////////////////////////////
                                        ...
                                        $parish = (isset($data->sheets[0]["cells"][$x][5]) && $data->sheets[0]["cells"][$x][5] !="") ? trim($data->sheets[0]["cells"][$x][5]) :"";
                                        if($parish !=''){
                                            $a5= $this->getSubCountyId($districtId,'Parish',$a1,$subcountyId,$parish);
                                            //debug($a5);
                                            if(!$a5){
                                                $parishArr = array(
                                                        'name'  => $parish,
                                                ...
                                                debug($parishArr);
                                                $this->Parish->saveAll($parishArr);
                                                echo $parishId = $this->Parish->id;
                                                $village = (isset($data->sheets[0]["cells"][$x][6]) && $data->sheets[0]["cells"][$x][6] !="") ? trim($data->sheets[0]["cells"][$x][6]) :"";
                                                if($village !=''){
                                                    $a6= $this->getSubCountyId($districtId,'Village',$a1,$subcountyId,$parishId,$village);
                                                    //debug($a6);
                                                    if(!$a6){
                                                        $villageArr = array(
                                                                'name'  => $village,
                                                ...
                                                $village = (isset($data->sheets[0]["cells"][$x][6]) && $data->sheets[0]["cells"][$x][6] !="") ? trim($data->sheets[0]["cells"][$x][6]) :"";
                                                if($village !=''){
                                                    $a6= $this->getSubCountyId($districtId,'Village',$a1,$subcountyId,$a5,$village);
                                                    //debug($a6);
                                                    if(!$a6){
                                                        $villageArr = array(
                                                                'name'  => $village,
                                    ...
                                    /////////////////////////////////////////Parish////////////////////////////////////////
                                        ...
                                        $parish = (isset($data->sheets[0]["cells"][$x][5]) && $data->sheets[0]["cells"][$x][5] !="") ? trim($data->sheets[0]["cells"][$x][5]) :"";
                                        if($parish !=''){
                                            $a5= $this->getSubCountyId($districtId,'Parish',$a1,$a3,$parish);
                                            //debug($a5);
                                            if(!$a5){
                                                $parishArr = array(
                                                        'name'  => $parish,
                    ...
                    $county = (isset($data->sheets[0]["cells"][$x][3]) && $data->sheets[0]["cells"][$x][3] !="") ? trim($data->sheets[0]["cells"][$x][3]) :"";
                    if($county !=''){
                        $a1= $this->getCountyId($a,'County',$county);
                        debug($a1);
                        if(!$a1){
                            $countyArr = array(
                                        'name'  => $county,
                            ...
                            $subCounty = (isset($data->sheets[0]["cells"][$x][4]) && $data->sheets[0]["cells"][$x][4] !="") ? trim($data->sheets[0]["cells"][$x][4]) :"";
                            if($subCounty !=''){
                                $a3= $this->getSubCountyId($a,'SubCounty',$countyId,$subCounty);
                                //debug($a);
                                if(!$a3){
                                    $subCountyArr = array(
                                                'name'  => $subCounty,
                                    ...
                                    /////////////////////////////////////////Parish////////////////////////////////////////
                                        ...
                                        $parish = (isset($data->sheets[0]["cells"][$x][5]) && $data->sheets[0]["cells"][$x][5] !="") ? trim($data->sheets[0]["cells"][$x][5]) :"";
                                        if($parish !=''){
                                            $a5= $this->getSubCountyId($a,'Parish',$county_id,$subcountyId,$parish);
                                            //debug($a5);
                                            if(!$a5){
                                                $parishArr = array(
                                                        'name'  => $parish,
                                                ...
                                                $village = (isset($data->sheets[0]["cells"][$x][6]) && $data->sheets[0]["cells"][$x][6] !="") ? trim($data->sheets[0]["cells"][$x][6]) :"";
                                                if($village !=''){
                                                    $a6= $this->getSubCountyId($a,'Village',$county_id,$subcountyId,$parishId,$village);
                                                    //debug($a6);
                                                    if(!$a6){
                                                        $villageArr = array(
                                                                'name'  => $village,
                                                ...
                                                $village = (isset($data->sheets[0]["cells"][$x][6]) && $data->sheets[0]["cells"][$x][6] !="") ? trim($data->sheets[0]["cells"][$x][6]) :"";
                                                if($village !=''){
                                                    $a6= $this->getSubCountyId($a,'Village',$county_id,$subcountyId,$a5,$village);
                                                    //debug($a6);
                                                    if(!$a6){
                                                        $villageArr = array(
                                                                'name'  => $village,
                                    ...
                                    /////////////////////////////////////////Parish////////////////////////////////////////
                                        ...
                                        $parish = (isset($data->sheets[0]["cells"][$x][5]) && $data->sheets[0]["cells"][$x][5] !="") ? trim($data->sheets[0]["cells"][$x][5]) :"";
                                        if($parish !=''){
                                            $a5= $this->getSubCountyId($a,'Parish',$county_id,$a3,$parish);
                                            //debug($a5);
                                            if(!$a5){
                                                $parishArr = array(
                                                        'name'  => $parish,
                                                ...
                                                if($village !=''){
                                                    ...
                                                    if(!$a6){
                                                        ...
                                                        debug($villageArr);
                                                        $this->Village->saveAll($villageArr);
                                                        echo $villageId = $this->Village->id;
                                                    }else{
                                                        echo 'Village already exist in row no '.$x.' of Village column';
                                                    }
                            ...
                            $subCounty = (isset($data->sheets[0]["cells"][$x][4]) && $data->sheets[0]["cells"][$x][4] !="") ? trim($data->sheets[0]["cells"][$x][4]) :"";
                            if($subCounty !=''){
                                $a3= $this->getSubCountyId($a,'SubCounty',$a1,$subCounty);
                                //debug($a);
                                if(!$a3){
                                    $subCountyArr = array(
                                                'name'  => $subCounty,
                                    ...
                                    /////////////////////////////////////////Parish////////////////////////////////////////
                                        ...
                                        $parish = (isset($data->sheets[0]["cells"][$x][5]) && $data->sheets[0]["cells"][$x][5] !="") ? trim($data->sheets[0]["cells"][$x][5]) :"";
                                        if($parish !=''){
                                            $a5= $this->getSubCountyId($a,'Parish',$a1,$subcountyId,$parish);
                                            //debug($a5);
                                            if(!$a5){
                                                $parishArr = array(
                                                        'name'  => $parish,
                                                ...
                                                $village = (isset($data->sheets[0]["cells"][$x][6]) && $data->sheets[0]["cells"][$x][6] !="") ? trim($data->sheets[0]["cells"][$x][6]) :"";
                                                if($village !=''){
                                                    $a6= $this->getSubCountyId($a,'Village',$a1,$subcountyId,$parishId,$village);
                                                    //debug($a6);
                                                    if(!$a6){
                                                        $villageArr = array(
                                                                'name'  => $village,
                                                ...
                                                $village = (isset($data->sheets[0]["cells"][$x][6]) && $data->sheets[0]["cells"][$x][6] !="") ? trim($data->sheets[0]["cells"][$x][6]) :"";
                                                if($village !=''){
                                                    $a6= $this->getSubCountyId($a,'Village',$a1,$subcountyId,$a5,$village);
                                                    //debug($a6);
                                                    if(!$a6){
                                                        $villageArr = array(
                                                                'name'  => $village,
                                    ...
                                    /////////////////////////////////////////Parish////////////////////////////////////////
                                        ...
                                        $parish = (isset($data->sheets[0]["cells"][$x][5]) && $data->sheets[0]["cells"][$x][5] !="") ? trim($data->sheets[0]["cells"][$x][5]) :"";
                                        if($parish !=''){
                                            $a5= $this->getSubCountyId($a,'Parish',$a1,$a3,$parish);
                                            //debug($a5);
                                            if(!$a5){
                                                $parishArr = array(
                                                        'name'  => $parish,
    ...
    public function getDistId($district,$type,$county='',$subCounty=''){
        $this->loadModel('District');
        $this->loadModel('County');
        $this->loadModel('SubCounty');
        if($type=='District'){
            //$a=$this->District->findByName($district);
            $a=$this->District->field('id',array('upper(District.name)'=>strtoupper($district)));
            return $a;
        }
        
    }
    public function getCountyId($district,$type,$county=''){
        $this->loadModel('District');
        $this->loadModel('County');
        $this->loadModel('SubCounty');
        if($type=='County'){
            //$a=$this->County->find('first',);
            $a1=$this->County->field('id',array('County.district_id'=>$district,'upper(County.name)'=>strtoupper($county)));
            return $a1;
        }
        
    }
    public function getSubCountyId($district,$type,$county='',$subCounty='',$parish='',$village=''){
        $this->loadModel('District');
        $this->loadModel('County');
        $this->loadModel('SubCounty');
        ...
        if($type=='SubCounty'){
            //$a=$this->County->find('first',);
            $a2=$this->SubCounty->field('id',array('SubCounty.district_id'=>$district,'SubCounty.county_id'=>$county,'upper(SubCounty.name)'=>strtoupper($subCounty)));
            return $a2;
        }
        if($type=='Parish'){
            //$a=$this->County->find('first',);
            $a3=$this->Parish->field('id',array('Parish.district_id'=>$district,'Parish.county_id'=>$county,'Parish.sub_county_id'=>$subCounty,'upper(Parish.name)'=>strtoupper($parish)));
            return $a3;
        }
        if($type=='Village'){
            //$a=$this->County->find('first',);
            $a4=$this->Village->field('id',array('Village.district_id'=>$district,'Village.county_id'=>$county,'Village.sub_county_id'=>$subCounty,'Village.parish_id'=>$parish,'upper(Village.name)'=>strtoupper($village)));
            return $a4;
        }
